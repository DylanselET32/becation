...
CREATE TABLE IF NOT EXISTS `becation_db`.`vacation` (
    ...
    `area_manager_authorization` TINYINT(1)  DEFAULT null COMMENT 'el jefe del area autoriza o no al empleado',
    `to_create` TIMESTAMP NOT NULL COMMENT 'marca temporal automatica de cuando se crea este registro en le DB\ndato predeterminado: CURRENT_TIMESTAMP    ',
...
INSERT INTO `becation_db`.`employer`(`user_id`, `available_days`, `total_days`, `is_cumulative`, `role_id`, `area_id`) VALUES(1, 10, 15, 0, 1, 1);

INSERT INTO `becation_db`.`user`(`name`, `surname`, `email`, `password`, `dni`, `privileges`) VALUES("Gonzalo", "Carranza", "gonzalocarranza@streambe.com", "gonzac123", 11223345, 1);
