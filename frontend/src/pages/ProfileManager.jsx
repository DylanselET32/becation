...
export default function Home({auth}){
    ...
    const fetchUsers = async () => {
        try {
          console.log("PRUEBAA")
            ...
            if(users.status !== 200) throw new Error("Error de servidor, intentar más tarde");
            let temporalUsers = [];
            users.data.map((event)=>{
                const users = {
                    allDay: true,
                    color: "grey",
                    editable: false,
                ...
                temporalUsers.push(users);
            });
            console.log("DATA:",users.data);
            setFetchData(users.data);
            setVacationDaysAsked(temporalUsers);
            setFetchDataToCalendara(temporalUsers)
        } catch (error) {
            setAlertConfig({
                show: true,
                status: 'danger',
    ...
    // Función para formatear los usuarios en la tabla
    const formatUsersToTable = (users) => {
        let temporalUsers = [];
        const status = {
            aproved: "Aprobado",
            denied: "Denegado",
